...
// 前馈力矩参数
#define FEEDFORWARD_TORQUE_VALUE 1.2f  // 前馈力矩大小 (Nm)
...
void Start_Alternating_Speed(void) {
    ...
    // 启动交替行走时Kd值由当前模式的current_limit决定（在运行时动态设置）
    // control_kd[0] = 2.0f;  // 不再固定设置，改为动态
    // control_kd[1] = 2.0f;
    ...
    // 计算初始前馈力矩（周期1状态）
    float initial_feedforward_1 = 0.0f, initial_feedforward_2 = 0.0f;
    if (current_walking_mode == MODE_FLAT_GROUND || current_walking_mode == MODE_STAIRS) {
        initial_feedforward_1 = FEEDFORWARD_TORQUE_VALUE;   // 电机1在周期1提供1Nm
        initial_feedforward_2 = FEEDFORWARD_TORQUE_VALUE;   // 电机2在周期1提供1Nm
    }

    // 使用当前模式的current_limit作为初始Kd值
    float initial_kd = speed_current_limit;

    float total_torque_1 = control_torque[0] + initial_feedforward_1;
    float total_torque_2 = control_torque[1] + initial_feedforward_2;

    // 临界区保护，立即发送电机控制指令
    portMUX_TYPE mux = portMUX_INITIALIZER_UNLOCKED;

    portENTER_CRITICAL(&mux);
    Motor_ControlMode(&motors[0], total_torque_1, control_position[0], initial_speed_1, control_kp[0], initial_kd);
    portEXIT_CRITICAL(&mux);

    vTaskDelay(pdMS_TO_TICKS(10)); // 10ms间隔

    portENTER_CRITICAL(&mux);
    Motor_ControlMode(&motors[1], total_torque_2, control_position[1], initial_speed_2, control_kp[1], initial_kd);
    portEXIT_CRITICAL(&mux);

    // 更新已发送参数缓存
    Update_Sent_Params(0, total_torque_1, control_position[0], initial_speed_1, control_kp[0], initial_kd);
    Update_Sent_Params(1, total_torque_2, control_position[1], initial_speed_2, control_kp[1], initial_kd);

    ESP_LOGI(TAG, "交替速度模式已启动 - 立即发送控制指令: 电机1速度=%.2f力矩=%.2f(Kd=%.1f), 电机2速度=%.2f力矩=%.2f(Kd=%.1f), 每%.1f秒切换",
             initial_speed_1, total_torque_1, initial_kd, initial_speed_2, total_torque_2, initial_kd, alternating_interval_ms / 1000.0f);
}
