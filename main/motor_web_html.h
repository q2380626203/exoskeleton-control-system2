...
"</div>"
"<div class='control-group'>"
"<label>抬腿最大持续时间(ms):</label>"
"<input type='number' id='liftMaxDuration' step='10' value='800' min='100' max='2000'>"
"</div>"
...
"}"
"}"
"if(data.vt_lift_max_duration!==undefined){"
"const liftMaxDurationInput=document.getElementById('liftMaxDuration');"
"if(document.activeElement!==liftMaxDurationInput){"
"liftMaxDurationInput.value=data.vt_lift_max_duration;"
"}"
"}"
...
"liftMaxDuration:parseFloat(document.getElementById('liftMaxDuration').value)"
"};"
"fetch('/api/params',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(params)}).then(r=>r.text()).then(alert)"
"}"
